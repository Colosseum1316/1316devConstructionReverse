...
public class CommandManager extends Module implements CommandExecutor {
  ...
  public boolean onCommand(
    ...
    for (int i = args.length; i >= 0; i--) {
      ...
      for (int x = 0; x < i; x++) sb.append(".").append(args[x].toLowerCase());
      ...
      if (commandMap.containsKey(cmdLabel)) {
        ...
        if (command.inGameOnly() && !(sender instanceof Player)) {
          sender.sendMessage(ChatColor.RED + "This command can only be preformed in game.");
          return true;
        }
        try {
          method.invoke(
    ...
    sender.sendMessage(
        ChatColor.RED
            + "I'm sorry, it looks like this command has not been handled. Please contact the server administrators.");
    ...
    for (Method m : obj.getClass().getMethods()) {
      if (m.getAnnotation(Command.class) != null) {
        ...
        if (m.getParameterTypes().length > 1 || m.getParameterTypes()[0] != CommandArgs.class) {
          System.out.println(
              "Unable to register command " + m.getName() + ". Unexpected method arguments");
          continue;
        }
        registerCommand(command, command.name(), m, obj);
        for (String alias : command.aliases()) registerCommand(command, alias, m, obj);
        ...
        if (m.getReturnType() != List.class) {
          System.out.println(
              "Unable to register tab completer " + m.getName() + ". Unexpected return type");
          continue;
        }
        registerCompleter(comp.name(), m, obj);
        for (String alias : comp.aliases()) registerCompleter(alias, m, obj);
      }
    }
  }
  ...
  public void registerCommand(Command command, String label, Method m, Object obj) {
    ...
    if (map.getCommand(cmdLabel) == null)
      map.register(plugin.getName(), new BukkitCommand(cmdLabel, this, plugin));
  ...
  public void registerCompleter(String label, Method m, Object obj) {
    ...
    if (map.getCommand(cmdLabel) instanceof BukkitCommand) {
      ...
      if (command.completer == null) command.completer = new BukkitCompleter();
      ...
      try {
        ...
        if (field.get(command) == null)
          field.set(command, new BukkitCompleter().addCompleter(label, m, obj));
        else if (field.get(command) instanceof BukkitCompleter)
          ((BukkitCompleter) field.get(command)).addCompleter(label, m, obj);
        else
          System.out.println(
              "Unable to register tab completer "
                  + m.getName()
