...
public final class World {
  ...
  public Block getBlock(Location location) throws Exception {
    AnvilChunk chunk = getChunk(getChunkAccess(), location.getBlockX(), location.getBlockZ());
  ...
  public void addAndSetSign(Location baseBlockLocation, String[] lines) throws Exception {
    ...
    for (int i = 0; i < lines.length; i++) {
      signTag.setString(AnvilFormat.TileEntityTag.SIGN_LINE_NAMES.get(i), Objects.requireNonNullElse(lines[i], ""));
    }
